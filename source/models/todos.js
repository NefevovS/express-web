...
export async function getList(user, doneAtLast, search) {
    ...
    if (doneAtLast === "1") qTodos.sort("done createdAt")
    else qTodos.sort("createdAt")
...
export async function getMostActiveUsers() {
    ...
    result[0] = await Todo.aggregate([{
        $lookup: {
            from: "users", localField: "user", foreignField: "_id", as: "userObj"
        }
    }, {
        $unwind: "$userObj"
    }, {
        $group: {
            _id: "$userObj.username", cnt: {$count: {}}
        }
    }, {$sort: {cnt: -1}}, {$limit: 3}])
    result[1] = await Todo.aggregate([{$match: {done: true}}, {
        $lookup: {
            from: "users", localField: "user", foreignField: "_id", as: "userObj"
        }
    }, {$unwind: "$userObj"}, {$group: {_id: "$userObj.username", cnt: {$count: {}}}}, {$sort: {cnt: -1}}, {$limit: 3}])
